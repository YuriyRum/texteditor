...
import { Paperclip, FileText, FileSpreadsheet, Image as ImageIcon, File, Check } from 'lucide-react';
...
export const AttachmentContextMenu: React.FC<AttachmentContextMenuProps> = ({
  ...
  const menuWidth = 260;
  const menuHeight = Math.min(320, attachments.length * 48 + 60);
  ...
  return (
    <div
      ...
      className="fixed z-50 w-64 bg-white/95 backdrop-blur-md rounded-xl shadow-2xl border border-slate-200/90 py-1.5 animate-in fade-in zoom-in-95 duration-100 select-none overflow-hidden"
    >
